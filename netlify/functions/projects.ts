...
export const handler: Handler = async (event) => {
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const docs = await col.find({ user_id: claims.sub }).sort({ created_at: -1 }).toArray();
      return jsonResponse(200, docs.map(toProject));
    }
    ...
    if (event.httpMethod === 'POST') {
      const body = await readJson<Partial<ProjectDoc> & { user_id?: string; title?: string }>(event);
      if (!body.user_id || !body.title) return errorResponse(400, 'Missing required fields: user_id, title');
      ...
      const doc: ProjectDoc = {
        ...
        progress_percentage: typeof body.progress_percentage === 'number' ? body.progress_percentage : 0,
