...
export const handler: Handler = async (event) => {
  try {
    ...
    try {
      ...
      return errorResponse(400, 'Invalid phone number format. Please use a valid US phone number.');
    }

    // Check if user already has verified phone
    const existingProfile = await profiles.findOne({ auth_user_id: claims.sub });
    if (existingProfile?.phone_verified && existingProfile?.phone === normalizedPhone) {
    ...
    if (recentCode) {
      ...
      if (minutesRemaining > 0) {
        return errorResponse(429, `Please wait ${minutesRemaining} minute(s) before requesting a new code`);
      }
    }
    ...
    try {
      ...
      return errorResponse(500, 'Failed to send verification code. Please try again later.');
    }
  } catch (err) {
