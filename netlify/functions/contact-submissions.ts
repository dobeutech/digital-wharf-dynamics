...
export const handler: Handler = async (event) => {
  try {
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (name.length < 2) return errorResponse(400, 'Name must be at least 2 characters');
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return errorResponse(400, 'Please enter a valid email address');
      if (message.length < 10) return errorResponse(400, 'Message must be at least 10 characters');
      if (phone && phone.length > 20) return errorResponse(400, 'Phone must be less than 20 characters');

      const smsConsent = Boolean(body.smsConsent);
      if (phone && !smsConsent) return errorResponse(400, 'SMS consent is required when providing a phone number');
      ...
      return jsonResponse(200, { success: true, submission: toSubmission(doc) });
    }
    ...
    if (event.httpMethod === 'PATCH') {
      ...
      const body = await readJson<Partial<ContactSubmissionDoc> & { status?: string }>(event);
      const now = new Date().toISOString();
