...
function getDeadlineStatus(deadline: string | null) {
  if (!deadline) return { color: "text-muted-foreground", label: "No deadline" };
  const daysRemaining = differenceInDays(new Date(deadline), new Date());
  if (isPast(new Date(deadline))) return { color: "text-red-500", label: `${Math.abs(daysRemaining)} days overdue`, icon: AlertTriangle };
  if (daysRemaining <= 7) return { color: "text-red-500", label: `${daysRemaining} days left`, icon: AlertTriangle };
  if (daysRemaining <= 14) return { color: "text-yellow-500", label: `${daysRemaining} days left`, icon: Clock };
  return { color: "text-green-500", label: `${daysRemaining} days left`, icon: CheckCircle };
}
...
export default function AdminCCPA() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<CCPARequest | null>(null);
    ...
    <div className="min-h-screen pt-24 pb-20 px-4">
      <div className="container mx-auto max-w-7xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Manage California Consumer Privacy Act requests</p>
        </div>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">No CCPA requests found</div>
            ) : (
                ...
                <Table>
                  ...
                  <TableBody>
                    {requests.map((request) => {
                      const deadline = getDeadlineStatus(request.response_deadline);
                      const DeadlineIcon = deadline.icon;
                      return (
                        <TableRow key={request.id}>
                          <TableCell className="font-mono text-sm">{request.reference_id}</TableCell>
            ...
            {selectedRequest && (
              <>
                <SheetHeader>
                  ...
                  <SheetDescription>{selectedRequest.reference_id}</SheetDescription>
                </SheetHeader>
                <div className="mt-6 space-y-6">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedRequest.submitted_at 
                          ? format(new Date(selectedRequest.submitted_at), "MMM d, yyyy h:mm a")
                          : "Unknown"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground mb-2">Request Types</p>
                  ...
                  {selectedRequest.additional_info && (
                    <div>
                      ...
                      <p className="font-medium whitespace-pre-wrap">{selectedRequest.additional_info}</p>
                    </div>
                  )}
                  ...
                  {selectedRequest.response_deadline && (
                    <div>
                      ...
                      <p className={`font-medium ${getDeadlineStatus(selectedRequest.response_deadline).color}`}>
                        {format(new Date(selectedRequest.response_deadline), "MMMM d, yyyy")}
                        {" - "}
                        {getDeadlineStatus(selectedRequest.response_deadline).label}
                      </p>
                    </div>
                  )}
                  ...
                  {selectedRequest.processed_at && (
                    <div>
                      ...
                      <p className="font-medium text-green-500">
                        {format(new Date(selectedRequest.processed_at), "MMM d, yyyy h:mm a")}
                      </p>
                    </div>
                  )}
