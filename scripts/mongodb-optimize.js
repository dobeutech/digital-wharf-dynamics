...
async function createIndexes(db) {
  ...
  const indexes = [
    ...
    // Project Tasks collection
    {
      ...
      indexes: [
        { key: { projectId: 1, status: 1 }, options: { name: 'project_status' } },
...
async function addTTLIndexes(db) {
  ...
  for (const { collection, field, expireAfterSeconds, name } of ttlIndexes) {
    try {
      ...
      console.log(`  ✅ Created TTL index on ${collection}.${field} (${expireAfterSeconds / 86400} days)`);
    } catch (error) {
      if (error.code === 85 || error.code === 86) {
        console.log(`  ℹ️  TTL index already exists on ${collection}.${field}`);
      } else {
        console.error(`  ❌ Failed to create TTL index on ${collection}:`, error.message);
      }
    }
  }
...
async function analyzeCollections(db) {
  ...
  for (const collInfo of collections) {
    try {
      ...
      const avgDocSize = stats.count > 0 ? (stats.size / stats.count).toFixed(0) : 0;
