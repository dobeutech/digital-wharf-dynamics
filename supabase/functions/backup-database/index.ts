...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Backup completed: ${metadata.recordCount} records from ${BACKUP_CONFIG.tables.length} tables`);
    ...
    return new Response(
      JSON.stringify({ error: 'Backup failed', details: error instanceof Error ? error.message : 'Unknown error' }),
